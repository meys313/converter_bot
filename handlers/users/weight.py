...
from keyboards.inline import weight_keyboard
from states import WeightStates

@dp.message_handler(Command('weight'), state = "*")
async def discount(message: types.Message, state: FSMContext):
    await message.answer('выберите величину из которой вы хотите перевести', reply_markup=weight_keyboard)
    await state.finish()
    await state.set_state(WeightStates.convert_from.state)

@dp.callback_query_handler(callback.filter(), state=WeightStates.convert_from)
async def get_from(call: CallbackQuery,  callback_data: dict, state: FSMContext):
    ...
    message_keyboards = await call.message.answer('выберите величину в которую вы хотите перевести', reply_markup=weight_keyboard)
    ...
    await WeightStates.next()
@dp.callback_query_handler(callback.filter(), state=WeightStates.convert_to)
...
@dp.message_handler(state=WeightStates.value)
